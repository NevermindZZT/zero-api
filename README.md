...
```bash
# 1. 创建配置目录并下载默认配置
mkdir -p zero-api && cd zero-api
mkdir -p data certs configs
curl -o configs/config.yaml https://raw.githubusercontent.com/NevermindZZT/zero-api/main/configs/config.yaml

# 2. 创建 docker-compose.yml
cat > docker-compose.yml << 'EOF'
services:
  zero-api:
    image: nevermindzzt/zero-api:latest
